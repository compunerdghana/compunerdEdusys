...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin.from("staff_training_records").insert({
...
export async function GET(req: NextRequest) {
  ...
  let q = admin.from("staff_training_records").select("*").eq("school_id", schoolId).order("start_date", { ascending: false });
