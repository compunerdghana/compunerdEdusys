...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin.from("staff_promotion_history").insert({
  ...
  await admin.from("staff_details").update({ designation: new_designation }).eq("profile_id", profile_id).eq("school_id", schoolId);
