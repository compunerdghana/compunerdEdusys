...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await getAdmin().from("staff_transfer_records").insert({
  ...
  await getAdmin().from("staff_details").update({ department: new_department }).eq("profile_id", profile_id).eq("school_id", schoolId);
