...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin.from("staff_exit_records").insert({
  ...
  await admin.from("profiles").update({ is_active: false }).eq("id", profile_id);
  ...
  await admin.from("staff_details").update({ employment_status: statusMap[exit_type] ?? "inactive" }).eq("profile_id", profile_id);
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await admin.from("staff_exit_records").update({ clearance_status }).eq("id", id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ data });
}
