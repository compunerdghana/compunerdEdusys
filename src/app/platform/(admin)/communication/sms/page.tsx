...
import { MessageSquare, Send, Loader2, FileText, Users, RefreshCw } from "lucide-react";
import { SlidePanel } from "@/components/ui/SlidePanel";
import { useToast } from "@/components/ui/Toast";

interface Template { id: string; name: string; body: string; }
interface Message { id: string; school_name: string; recipient_name: string; recipient_phone: string; body: string; status: string; created_at: string; provider: string; }
...
export default function SMSCenterPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [msgs, tmpls] = await Promise.all([
      fetch("/api/platform/communication/sms").then(r => r.json()).catch(() => ({ messages: [] })),
      fetch("/api/platform/communication/templates?channel=sms").then(r => r.json()).catch(() => ({ templates: [] })),
    ]);
  ...
  const smsCount = Math.ceil((form.message.length || 1) / 160);

  async function handleSend() {
    if (!form.message) { toastError("Message is required."); return; }
    setLoading(true);
    try {
      const res = await fetch("/api/platform/communication/sms", {
        method: "POST",
      ...
      success(`SMS sent to ${data.sent} recipient${data.sent !== 1 ? "s" : ""}. Failed: ${data.failed}.`);
      setPanelOpen(false);
      setForm({ target_audience: "all", recipient_phone: "", recipient_name: "", message: "", template_id: "" });
      load();
    } catch (e) {
      toastError(e instanceof Error ? e.message : "Failed to send SMS.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-[13px] font-semibold mt-1 ml-12">Send SMS via Hubtel, Arkesel, or Nsano.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-[#e8e4f3] shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-[#f0edf8]">
          <h3 className="font-extrabold text-slate-900 text-[14px]">Sent SMS Messages</h3>
        </div>
        {fetching ? (
          <div className="flex items-center justify-center py-16">
            <div className="w-7 h-7 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: "#e0daf0", borderTopColor: "#3b82f6" }} />
          </div>
        ) : messages.length === 0 ? (
          <div className="flex flex-col items-center py-16 gap-3">
            ...
            <p className="text-slate-500 font-semibold text-[13px]">No SMS messages sent yet.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#faf9ff]">
                  {["School / Recipient", "Phone", "Message", "Provider", "Status", "Date"].map(h => (
              ...
              <tbody className="divide-y divide-[#f8f6ff]">
                {messages.map(msg => (
                  <tr key={msg.id} className="hover:bg-[#faf9ff] transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-[10px] font-extrabold uppercase px-2 py-1 rounded-full bg-slate-50 text-slate-500 border border-slate-100">{msg.provider}</span>
                    </td>
                    <td className="px-5 py-3.5">
      ...
      {/* Send Panel */}
      <SlidePanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Send SMS" subtitle="Send text messages to schools" width="lg">
        <div className="space-y-4">
          ...
          <div className="p-3 rounded-xl border border-dashed border-blue-200 bg-blue-50/40">
            <p className="text-[11px] font-bold text-blue-700 mb-3">— OR send to a specific number —</p>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Type your SMS message. Use {school_name} for personalization…"
              rows={5}
              maxLength={1600}
            ...
            <div className="flex items-center justify-between mt-1">
              <p className="text-[10px] text-slate-400 font-semibold">{form.message.length} / 1600 characters</p>
              <p className="text-[10px] font-bold" style={{ color: smsCount > 1 ? "#f59e0b" : "#94a3b8" }}>{smsCount} SMS part{smsCount !== 1 ? "s" : ""}</p>
            </div>
          </div>
          ...
          <button
            ...
            style={{ background: "linear-gradient(135deg,#3b82f6,#1d4ed8)" }}
          >
            {loading ? <><Loader2 size={14} className="animate-spin" /> Sending…</> : <><Send size={14} /> Send SMS</>}
          </button>
        </div>
      </SlidePanel>
    </div>
